...
bot = telebot.TeleBot(token='6641987391:AAEjgnZcxnwkMx8bgzgyGy-BzzBdG30Al1Y', parse_mode='html')
...
def welcome(message):
	...
	reply = bot.send_message(message.chat.id, f"Привет, <b>{username}</b>! 👋🏻\nМеня зовут Luffy! Я бот-генератор тестовых файлов. Помогу тебе проверить граничные значения при загрузке файлов разного веса в приложениях и на веб-сайтах. Я умею генерировать файлы различных расширений размерами от 1 байта до 45 мегабайт включительно.\n\nЧтобы начать, выбери одно из доступных расширений в меню ниже ⬇️", reply_markup=markup)
